<template>
    <div class="mt-[176px]">
        <div id="services" class="text-center w-full">
            <!-- <p class="font-[Bnim] text-[44px] uppercase">Lorem Ipsum is simply</p> -->
            <p class="font-[Bnim] text-[28px] md:text-[78px] uppercase">Our Products</p>
            <p class=" text-[18px] text-gray-600">Our products combine cutting-edge technologies in Artificial
                Intelligence, Internet of Things, and Augmented Reality</p>
        </div>




        <div
            class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3  mx-auto w-fit gap-4 justify-center mt-[120px]">
            <div class="w-full lg:w-[100%]">
                <img class="w-[400px] h-auto md:h-[267px] rounded-t-[27px]"
                    src="https://plus.unsplash.com/premium_photo-1733342533441-c4309b51da17?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=1032"
                    alt="">
                <div class=" bg-[white] p-[15px] w-full lg:w-[400px] md:w-[476px] flex flex-col gap-y-[15px] pt-[35px]">
                    <h1 class="text-[22px] md:text-[30px] font-bold">AI Solutions</h1>
                    <p class="text-[18px] md:text-[20px] text-gray-600">Empowering businesses with intelligent
                        automation and data-driven insights.
                    </p>
                    <div class="text-white bg-[#BE100F] px-[70px] w-fit py-2 rounded-[10px] text-center">
                        <a class="text-[16px] font-semibold" href="">More</a>
                    </div>
                </div>
            </div>

            <div class="w-full lg:w-[100%]">
                <img class="w-[400px] h-auto md:h-[267px] rounded-t-[27px]"
                    src="https://plus.unsplash.com/premium_photo-1733342533441-c4309b51da17?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=1032"
                    alt="">
                <div class=" bg-[white] p-[15px] w-full lg:w-[400px] md:w-[476px] flex flex-col gap-y-[15px] pt-[35px]">
                    <h1 class="text-[22px] md:text-[30px] font-bold">AI Solutions</h1>
                    <p class="text-[18px] md:text-[20px] text-gray-600">Empowering businesses with intelligent
                        automation and data-driven insights.
                    </p>
                    <div class="text-white bg-[#BE100F] px-[70px] w-fit py-2 rounded-[10px] text-center">
                        <a class="text-[16px] font-semibold" href="">More</a>
                    </div>
                </div>
            </div>

            <div class="w-full lg:w-[100%]">
                <img class="w-[400px] h-auto md:h-[267px] rounded-t-[27px]"
                    src="https://plus.unsplash.com/premium_photo-1733342533441-c4309b51da17?ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&q=80&w=1032"
                    alt="">
                <div class=" bg-[white] p-[15px] w-full lg:w-[400px] md:w-[476px] flex flex-col gap-y-[15px] pt-[35px]">
                    <h1 class="text-[22px] md:text-[30px] font-bold">AI Solutions</h1>
                    <p class="text-[18px] md:text-[20px] text-gray-600">Empowering businesses with intelligent
                        automation and data-driven insights.
                    </p>
                    <div class="text-white bg-[#BE100F] px-[70px] w-fit py-2 rounded-[10px] text-center">
                        <a class="text-[16px] font-semibold" href="">More</a>
                    </div>
                </div>
            </div>



        </div>

    </div>
</template>