...
const save = async () => {
    ...
    try {
        await api.put(`/admin/activity/${id}`, fd);
...
<template>
    <div class="space-y-5">
        ...
        <div class="flex gap-2">
            <button @click="save" class="bg-green-600 hover:bg-green-500 text-white px-4 py-2 rounded">{{ $t('dashboard.save-all') }}</button>
            <button @click="deleteArticle" class="bg-red-600 hover:bg-red-500 text-white px-4 py-2 rounded">{{ $t('dashboard.delete') }}</button>
        </div>
    </div>
</template>
